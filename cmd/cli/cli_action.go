...
var dronePlugin *file_browser_plugin.FileBrowserPlugin

// GlobalBeforeAction
// do command Action before flag global.
func GlobalBeforeAction(c *cli.Context) error {
	isDebug := file_browser_plugin.IsBuildDebugOpen(c)
	if isDebug {
		drone_log.OpenDebug()
	}
	...
	p := file_browser_plugin.BindFlag(c, cliVersion, pkgJson.GetPackageJsonName(), drone)
	dronePlugin = &p
